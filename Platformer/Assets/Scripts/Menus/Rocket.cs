...
* Control flying rocket in menu.scene
*/
...
namespace Menus
{
    public class Rocket : MonoBehaviour
    {
        ...
        void FixedUpdate()
        {
            ...
            {
                gameObject.GetComponent<Image>().sprite = rocket[currentstate/20]; //show the matching image
                currentstate++;
            ...
            if (r_rigidbody.position.y > border & direction_dec < 2 ^ r_rigidbody.position.y < border & direction_dec >=2) //check border dependent on the direction
            {
                ...
                if (direction_dec == 0) //down right to up left
                {
                ...
                else if (direction_dec == 1) //down left to up right
                {
                ...
                else if (direction_dec == 2) //up left to down right
                {
        ...
        void ResetValues()
        {
            startposition = (new Vector3(1440f, -1080f, 0)); //set startposition
            direction = new Vector3(-1.0f, 1.0f, 0.0f); //set direction
            border=cam.ScreenToWorldPoint(new Vector3(0,2160,0)).y; //set border
        }
    }
}